 function [TrainFeatures, TestFeatures, trainLabs, testLabs, trainSeq, testSeq] = CV_LeavePOut_func(TrainFeatures, TestFeatures, trainLabs, testLabs, trainSeq, testSeq, n, fold_size, length, f)

%% Leave p-out Cross-validation: 
%For each model: Divide Features into n folds, and select one(index for
%testing features in a fold decided by input f)
%%in each fold as test features, perform classification x(equal to size of fold) times with
%%repeat of testing features
...
for i = 1:n
    ...
    %Decide test Feature index(location) in each fold with input 'f'
    test_indice = f;
